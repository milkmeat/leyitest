...
## 任务
- A = [ 20010413, 20010414, 20010415, 20010416, 20010417 ], 20010413是队长(A1)
- B = 20010373
测试流程如下
- 先给A,B都加上足够的宝石
...
- A1对B发起侦察（scout_player），只需要发起动作，不需要读取侦���结果
- A1对B发起攻击(create_rally)，默认倒计时为5分钟。只带5000士兵，不带英雄，用于跑通流程
- A组其余成员对A1发起增援（join_rally），也是只派出5000士兵
- 等待6分钟至战斗完成
- 验证A,B的士兵数量都减少了
...
- ✅ join_rally CLI 参数格式已从 `<uid> <rally_id>` 改为 `<uid> <rally_id> [soldier_id count]`
...
### 6. ⬜ Test server 验证（待用户配置联盟）
- 用户需要在 test server 上将 A1-A5 加入同一联盟
- 之后运行 `./test_rally.sh` 验证完整流程（含战斗结算）

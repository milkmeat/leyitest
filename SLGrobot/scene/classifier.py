...
class SceneClassifier:
    ...
    def get_confidence(self, screenshot: np.ndarray) -> dict[str, float]:
        ...
        triangle_match = self.template_matcher.match_one(screenshot, "down_triangle")
        ...
        main_city_match = self.template_matcher.match_one(corner, "world")
        world_map_match = self.template_matcher.match_one(corner, "territory")
        ...
        # 6. Secondary: check scene-specific templates on full screenshot.
        #    With flat template directory, try each known scene name directly.
        for scene_name in self._scenes:
            if scene_name in ("main_city", "world_map", "popup",
                              "loading", "story_dialogue", "unknown",
                              "exit_dialog", "battle"):
                continue  # Already handled above or no template
            match = self.template_matcher.match_one(screenshot, scene_name)
            if match is not None:
                current = scores.get(scene_name, 0.0)
                scores[scene_name] = max(current, match.confidence)
    ...
    def _detect_popup_score(self, screenshot: np.ndarray) -> float:
        ...
        if center_mean > 50 and border_mean < center_mean * 0.5:
            # Also check for close_x button template
            close_match = self.template_matcher.match_one(screenshot, "close_x")
            if close_match is not None:
                return max(0.9, close_match.confidence)
            return 0.7
    ...
    def _detect_exit_dialog_score(self, screenshot: np.ndarray) -> float:
        ...
        """
        match = self.template_matcher.match_one(screenshot, "exit_dialog")
